...
int main(int argc, char **argv)
{
    ...
    if (argc > 1 && strcasecmp(argv[1], "engr") == 0) {
        /* 进入施工模式 */
    ...
    if (tuya_get_engineer_mode()) {
        /* 创建施工的存储路径 */
        TUYA_CALL_ERR_RETURN(user_engr_set_storage_path(cfg->storage_dir));
        storage_path = user_engr_get_storage_path();
    } else {
        storage_path = cfg->storage_dir;
    }
    TUYA_CALL_ERR_RETURN(tuya_iot_init(storage_path));
    ...
    /* 私有业务初始化 */
    TUYA_CALL_ERR_RETURN(user_engr_svc_init(cfg->product_id));
    TUYA_CALL_ERR_RETURN(user_zigbee_svc_init());

    ty_cJSON *json_pid_engr = ty_cJSON_GetObjectItem(cfg->input, "pid_engr");
    if (json_pid_engr && json_pid_engr->type == ty_cJSON_String) {
        pid_engr = json_pid_engr->valuestring;
        PR_INFO("engr pid: %s", pid_engr);
    }
    ...
    if (tuya_get_engineer_mode()) {
        tuya_engineer_gw_init(IOT_GW_NET_WIRED_WIFI, pid_engr, USER_SW_VER, NULL, 0);
    } else {
...
#else
    if (tuya_get_engineer_mode()) {
        tuya_engineer_gw_init(IOT_GW_NET_WIFI, pid_engr, USER_SW_VER, NULL, 0);
    } else {
...
#else
    if (tuya_get_engineer_mode()) {
        tuya_engineer_gw_init(IOT_GW_NET_WIRED, pid_engr, USER_SW_VER, NULL, 0);
    } else {
